...
export default function StatsCarouselCount({
  ...
  const [phase, setPhase] = useState<"idle" | "down" | "stackUp" | "upReenter">("idle");
        ...
        <AnimatePresence>
          {items.map((stat, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex flex-col items-center justify-center rounded-xl border border-border p-6 bg-white dark:bg-zinc-900 shadow-lg ${isDragMode && isTopCard ? "hover:shadow-xl transition-shadow duration-200" : ""} ${cardClassName}`}
                >
                  {shouldAutoAnimate ? (
                    <StatsCarousel
                      value={stat.value}
